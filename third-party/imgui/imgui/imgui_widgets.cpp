...
bool ImGui::ColorPicker3(const char* label, float col[3], ImGuiColorEditFlags flags)
{
    ...
    if (!ColorPicker5(label, col4, flags | ImGuiColorEditFlags_NoAlpha))
        return false;
...
void ImGui::DrawCircle(float PosX, float PosY, float Radius)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ImDrawList* draw_list = window->DrawList;
    ImVec2 sv_cursor_pos;
    sv_cursor_pos.x = PosX;
    sv_cursor_pos.y = PosY;
    draw_list->AddCircle(sv_cursor_pos, Radius, IM_COL32(0,255,0,255), 48);
    draw_list->AddCircle(sv_cursor_pos, Radius+1, IM_COL32(0,255,0,255), 48);
    draw_list->AddCircle(sv_cursor_pos, Radius+2, IM_COL32(0,255,0,255), 48);

}
// Note: ColorPicker4() only accesses 3 floats if ImGuiColorEditFlags_NoAlpha flag is set.
// FIXME: we adjust the big color square height based on item width, which may cause a flickering feedback loop (if automatic height makes a vertical scrollbar appears, affecting automatic width..)
bool ImGui::ColorPicker5(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ImDrawList* draw_list = window->DrawList;
    ...
    ImVec2 picker_pos = window->DC.CursorPosPrevLine;
    float square_sz = GetFrameHeight();
    float bars_width = square_sz; // Arbitrary smallish width of Hue/Alpha picking bars
    float sv_picker_size = ImMax(bars_width *1, CalcItemWidth()+30 - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value picking box
    float bar0_pos_x = picker_pos.x + sv_picker_size + style.ItemInnerSpacing.x;
    float bar1_pos_x = bar0_pos_x+80 + bars_width + style.ItemInnerSpacing.x;
