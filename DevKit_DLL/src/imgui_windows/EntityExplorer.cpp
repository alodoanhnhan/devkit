...
void EntityExplorer::Render() {
    ...
    for (std::map<unsigned __int32, CNPCPosData *>::iterator it = g_CGlobalDataManager->m_npcPosDataMap.begin();
         it != g_CGlobalDataManager->m_npcPosDataMap.end(); ++it) {

        ImGui::Text("%d", it->first);
        ImGui::NextColumn();
        ImGui::Text("%p", it->second);
        ImGui::NextColumn();
        ImGui::Text("%d", it->second->data.m_NpcID);
        ImGui::NextColumn();
        ImGui::Text("(%.2f, %.2f, %.2f)",
                    it->second->data.m_PosX,
                    it->second->data.m_PosY,
                    it->second->data.m_PosZ
        );
        ImGui::NextColumn();
        ImGui::Separator();
