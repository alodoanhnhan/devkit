...
void Inventory::Render() {
    ...
    for (int i = 0; i < 20; i++) {
       ...
       /* ImGui::Text("Item %d: %p [%d]", i, item, item->pBluesMap.size());

        if (item->pBluesMap.size() == 0)
            continue;


        for (std::n_map<short,int>::iterator it = item->pBluesMap.begin(); it != item->pBluesMap.end(); ++it) {
            switch (it->first) {
