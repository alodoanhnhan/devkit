...
#include <algorithm>
bool InterfaceTree::WholeChatShow;
...
void InterfaceTree::Renders() {

    CProcess *process = theApp.GetCurrentProcess();
    if (process != NULL) {
        RecurseElements(process);
    }

}
void InterfaceTree::RecurseElements(const CGWndBase *element) {
    ...
    for (std::n_list<CGWndBase *>::const_iterator it = element->N00000707.begin();
        ...
        if(elem->UniqueID()==56)
        {
            ...
            InterfaceTree::WholeChatShow = true;
            printf("%s\n", elem->GetRuntimeClass()->m_lpszClassName);

        } else
        {

                InterfaceTree::WholeChatShow = false;
                printf("aaaa%s\n", elem->GetRuntimeClass()->m_lpszClassName);


        }

         RecurseElements(elem);
