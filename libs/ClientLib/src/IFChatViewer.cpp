...
#include "IFChatWndCtrl.h"
#include "IFflorian0Guide.h"
#include "../../../DevKit_DLL/src/Util.h"
#include <sstream>
#include <GFXMainFrame/Controler.h>

CIFSlotWithHelp* CIFChatViewer::Slot;
CIFSlotWithHelp* CIFChatViewer::Slot2;
...
void CIFChatViewer::OnListChatThing(int a1, int a2) {
    ...
    ///////
    int id = GetIDOfInterfaceUnderCursor();
   // printf("GetIDOfInterfaceUnderCursor %d",id);
    CIFListCtrl *pList;
   ...
   // printf("Index %d \n", pList->GetNumberOfItems());

    std::n_wstring str;
    line->m_font->GetText(&str);
    std::string strs = TO_STRING(str).c_str();
    //printf("%s",strs.c_str());
    if (strs.rfind("_h_t_t_p") != std::n_wstring::npos)
    {
        std::string sender = strs.substr(0, strs.find(':'));

        size_t start_pos2 = 0;
        const std::string& from2="_";
        const std::string& to2="";
        while((start_pos2 = strs.find(from2, start_pos2)) != std::string::npos) {
            strs.replace(start_pos2, from2.length(), to2);
            start_pos2 += to2.length(); // Handles case where 'to' is a substring of 'from'
        }
        std::size_t pos = strs.find("http");      // position of "live" in str
        std::string origins = strs.substr (pos);     // get from "live" to the end
        m_UrlLink=origins;
        g_pCGInterface->GetCIFSTConfirmUrlGUI()->ShowGWnd(true);
        g_pCGInterface->GetCIFSTConfirmUrlGUI()->ResetPosition();
    }
...
__declspec (naked) void ASM_OnIndexHover()
{
    ...
    PUSH EBP;
    PUSH EDX;
    CALL CIFChatViewer::OnIndexHovered;
    POPFD;
    POPAD;
    CALL call1_addr;
...
void CIFChatViewer::Initialize()
{
    g_MemoryHelper->Detour(false, reinterpret_cast<void*>(cdwOnIndexHover), ASM_OnIndexHover);
}
void CIFChatViewer::OnIndexHovered(DWORD EDX, DWORD EBP)
{
    /////////////////////
    try
    {
        CIFChatWndCtrl* checker = 0;
        __asm mov checker, ebx;
        if(checker && checker->Instance == 0xD9E37C && g_pCICPlayer && g_CurrentIfUnderCursor->UniqueID()==16 && (CIFChatViewer*)g_pCGInterface->m_IRM.GetResObj(1, 1)->IsVisible())
        {
            ...
            if (!Slot2) {
                Slot2 = (CIFSlotWithHelp*)g_pCGInterface->CreateInstance(g_pCGInterface, GFX_RUNTIME_CLASS(CIFSlotWithHelp), rect, 5001, 0);
            ...
            int indexOffset = 0;
            //get the message that conatains the name of the player
            do
            {
                line = pList->GetTextByIndex(EDX - indexOffset);
                if (!line)
                    break;
                line->m_font->GetText(&str);
                if (str.empty())
            ...
            {
                line = pList->GetTextByIndex(EDX - i);
                if(line)
                {
                    line->m_font->GetText(&str);
                    fullStr = fullStr+str;
            ...
            std::string CharName;
            if(fullStr.find(whisper_from.c_str()) != std::string::npos)
                CharName = acp_encode(fullStr.substr(0, fullStr.find_first_of(L"(")));
            else if(fullStr.find(party.c_str()) != std::string::npos)
                CharName = acp_encode(fullStr.substr(0, fullStr.find_first_of(L"(")));
            else if(fullStr.find(guild.c_str()) != std::string::npos)
                CharName = acp_encode(fullStr.substr(0, fullStr.find_first_of(L"(")));
            else if(fullStr.find(tc.c_str()) != std::string::npos)
                CharName = acp_encode(fullStr.substr(0, fullStr.find_first_of(L"(")));
            else if(fullStr.find(whisper_to.c_str()) != std::string::npos)
                CharName = TO_STRING(g_pCICPlayer->GetCharName());
            else
                CharName = acp_encode(fullStr.substr(0, fullStr.find_first_of(L":")));
            ...
            int clickIndex= (mStartX + (indexOffset * 350)) / 6;
            ...
            if (startIndex > errorStartIndex&& endIndex<errorEndIndex|| errorStartIndex== std::string::npos|| errorEndIndex==std::string::npos) {
                if (startIndex != std::string::npos && endIndex != std::string::npos) {
                    ...
                    std::string nameinfo = CharName + ItemLinkName ;
                    size_t start_pos = 0;
                    const std::string &from = "     ";
                    const std::string &to = "";
                    while ((start_pos = nameinfo.find(from, start_pos)) != std::string::npos) {
                        nameinfo.replace(start_pos, from.length(), to);
                        start_pos += to.length(); // Handles case where 'to' is a substring of 'from'
                    }
                    //printf("%s\r\n",nameinfo.c_str());
                    if (ItemLinkInfoList.find(nameinfo) != ItemLinkInfoList.end() ) //item found
                    {
                        ...
                        if (isFound && pEmptySlot && !pEmptySlot->ItemInfo)
                        {
                            ...
                            if (pEmptySlot->ItemInfo == NULL && pEmptySlot->ItemInfo == 0)
                            {
                                ...
                                for (size_t i = 0; i < ItemLinkInfoList[nameinfo].buffer->GetLength(); ++i)
                                {
                                    UINT8 strLens ;
                                    ItemLinkInfoList[nameinfo].buffer->Read(&strLens, 1);
                                    pBuffer->Write(&strLens, 1);
                                }
                                ItemLinkInfoList[nameinfo].buffer->m_currentReadBytes=0;
                                ...
                                *(BYTE*)(0x0087C15B) = 0x74;
                                ...
                                Slot->ItemInfo = pEmptySlot->ItemInfo;
                    ...
                    if (ChatItemInfoList.find(nameinfo) != ChatItemInfoList.end() ) //item found
                    {
                        Slot2->ItemInfo=NULL;
                        Slot2->ItemInfo = &ChatItemInfoList[nameinfo].ItemInfo;
                        if (Slot2->ItemInfo && Slot2->ItemInfo !=0 && Slot2->ItemInfo !=NULL) {

                            *((DWORD*)Slot2->ItemInfo + 0x16)  = 7;
                            *((DWORD*)Slot2->ItemInfo + 0x15) = 0;
                            *((DWORD*)Slot2->ItemInfo + 0x11) = 0;
                            *((DWORD*)Slot2->ItemInfo + 0x30) = 7;
                            *((DWORD*)Slot2->ItemInfo + 0x2f) = 0;
                            *((DWORD*)Slot2->ItemInfo + 0x2b) = 0;
                            int v2 = reinterpret_cast<DWORD(__stdcall*)()>(0x5CED70)();

                            *((DWORD*)Slot2->ItemInfo + 0x32) = v2;
                            *(BYTE*)(v2 + 0x15) = 1;
                            *(DWORD *)(*((DWORD *)Slot2->ItemInfo + 0x32) + 4) = *((DWORD *)Slot2->ItemInfo + 0x32);
                            **((DWORD**)Slot2->ItemInfo + 0x32) = *((DWORD*)Slot2->ItemInfo + 0x32);
                            *(DWORD *)(*((DWORD *)Slot2->ItemInfo + 0x32) + 8) = *((DWORD *)Slot2->ItemInfo + 0x32);

                            *((DWORD*)Slot2->ItemInfo + 0x33)= 0;
                            *((DWORD*)Slot2->ItemInfo + 0x35)= 0;
                            *((DWORD*)Slot2->ItemInfo + 0x36)= 0;
                            *((DWORD*)Slot2->ItemInfo + 0x37)= 0;
                            // reinterpret_cast<void(__thiscall *)(DWORD*)>(0x008ba3d0)(((DWORD*)Slot->ItemInfo + 0x38));//show hide chi so
                            *((DWORD*)Slot2->ItemInfo + 0x52)= 0;
                            *((DWORD*)Slot2->ItemInfo + 0x53)= 0;
                            *((DWORD*)Slot2->ItemInfo + 0x54)= 0;
                            *((DWORD*)Slot2->ItemInfo + 0x56)= 0;
                            *((DWORD*)Slot2->ItemInfo + 0x57)= 0;
                            *((DWORD*)Slot2->ItemInfo + 0x58)= 0;
                            ...
                            g_pCGInterface->m_helperWindow->m_ownerWindow = Slot2;
                            g_pCGInterface->m_helperWindow->Reset();
                            g_pCGInterface->N00002717->Reset();
                            Slot2->HelperBubbleInventoryAndEquipment(g_pCGInterface->m_helperWindow);
