...
#endif
#define UNIQUE_SPAWN_SYSTEM_COLOR 0x0028FF00
#define UNIQUE_KILL_SYSTEM_COLOR 0xFFFF0000
#define DEFAULT_SYSTEM_COLOR 0xFFD5C588
void CNetProcessIn::RegisterPacketHandlers() {
    ...
    m_handlers.insert(std::make_pair(0x180C, &CNetProcessIn::OnChatItemInfo));
}
...
void CNetProcessIn::OnChatItemInfo(CMsgStreamBuffer& msg) {
    UINT16 strLen;
    msg.Read(&strLen, 2);

    char* charName = new char[strLen];
    msg.Read(charName, strLen);
    printf("OnChatItemInfo:%s\r\n", charName);
    CSOItem gold;
    msg.Read(&gold, 0x1D0);
    //ChatItemInfoList[charName] = gold;
}
void CNetProcessIn::OnGameNotify(MsgStreamBuffer* msg)
{
    CGInterface* g_interface = CGInterface::Get();
    CTextStringManager* g_textmanager = CTextStringManager::Get();

    if (msg && g_interface && g_textmanager)
    {

        if (CIFSystemMessage* sysmsg = g_interface->m_IRM.GetResObj<CIFSystemMessage>(68, 1))
        {
            unsigned short type = msg->Read<unsigned short>();
            std::wstring* chat;
            ...
            switch (type)
            {
                ...
                {
                    unsigned int UniqueID = msg->Read<unsigned int>();
                    ...
                    if (CGlobalDataManager* g_datamanager = CGlobalDataManager::Get())
                    {
                        if (CharData* uqdata = g_datamanager->GetCharData(UniqueID))
                        {
                            ...
                            std::wstring* UniqueName = g_textmanager->GetStringTextByCode(&uqdata->ObjNameStrID);

                            /*if (uqdata->ObjNameStrID.compare(L"SN_MOB_CH_TIGERWOMAN") == 0)
                            msg = g_textmanager->GetStringTextByCode(L"UIIT_MSG_APPEAR_TIGER_GIRL");
                            else if (uqdata->ObjNameStrID.compare(L"SN_MOB_KK_ISYUTARU") == 0)
                            msg = g_textmanager->GetStringTextByCode(L"UIIT_MSG_APPEAR_IYUTARU");
                            else if (uqdata->ObjNameStrID.compare(L"SN_MOB_OA_URUCHI") == 0)
                            msg = g_textmanager->GetStringTextByCode(L"UIIT_MSG_APPEAR_URUCHI");
                            else if (uqdata->ObjNameStrID.compare(L"SN_MOB_TK_BONELORD") == 0)
                            msg = g_textmanager->GetStringTextByCode(L"UIIT_MSG_APPEAR_BONELORD");
                            else if (uqdata->ObjNameStrID.compare(L"SN_MOB_RM_TAHOMET") == 0)
                            msg = g_textmanager->GetStringTextByCode(L"UIIT_MSG_APPEAR_TAHOMET");
                            else*/

                                wchar_t buffer[0x100];
                                swprintf_s(buffer, sizeof(buffer), g_textmanager->GetStringTextByCode(L"UIIT_MSG_APPEAR_UNIC")->c_str(), UniqueName->c_str());
                                g_interface->ShowBlueNotify(buffer);

                                sysmsg->WriteMessage(0xFF, UNIQUE_SPAWN_SYSTEM_COLOR, buffer, 1, 1);
                ...
                {
                    unsigned int UniqueID = msg->Read<unsigned int>();
                    std::wstring KillerName(msg->ReadStringW());
                    ...
                    if (CGlobalDataManager* g_datamanager = CGlobalDataManager::Get())
                    {
                        if (CharData* uqdata = g_datamanager->GetCharData(UniqueID))
                        {
                            ...
                            if (KillerName.compare(L"???") == 0)
                            {
                                    wchar_t buffer[0x100];
                                    swprintf_s(buffer, sizeof(buffer), g_textmanager->GetStringTextByCode(L"UIIT_MSG_DEAD_UNIC")->c_str(), UniqueName->c_str());
                                    g_interface->ShowBlueNotify(buffer);

                                    sysmsg->WriteMessage(0xFF, UNIQUE_KILL_SYSTEM_COLOR, buffer, 1, 1);
                            }
                            else
                            {
                                wchar_t buffer[0x100];

                                    swprintf_s(buffer, sizeof(buffer), g_textmanager->GetStringTextByCode(L"UIIT_MSG_ANYONE_DEAD_UNIC")->c_str(), KillerName.c_str(), UniqueName->c_str());
                                    g_interface->ShowBlueNotify(buffer);
                                    sysmsg->WriteMessage(0xFF, UNIQUE_KILL_SYSTEM_COLOR, buffer, 1, 1);
                ...
                case 0x0C07:
                {
                    unsigned char type = msg->Read<unsigned char>();
                    std::wstring guildname(msg->ReadStringW());

                    switch (type)
                    {
                        case 1:
                        {
                            chat = g_textmanager->GetStringTextByCode(L"UIIT_MSG_EVENT_051021_BADGEGAIN_BLUEDRAGON");
                            break;
                        }
                        case 2:
                        {
                            chat = g_textmanager->GetStringTextByCode(L"UIIT_MSG_EVENT_051021_BADGEGAIN_PHOENIX");
                            break;
                        }
                        case 3:
                        {
                            chat = g_textmanager->GetStringTextByCode(L"UIIT_MSG_EVENT_051021_BADGEGAIN_WHITETIGER");
                            break;
                        }
                        case 4:
                        {
                            chat = g_textmanager->GetStringTextByCode(L"UIIT_MSG_EVENT_051021_BADGEGAIN_TURTLE");
                            break;
                        }
                        case 5:
                        {
                    ...
                    wchar_t buffer[0x100];
                    swprintf_s(buffer, sizeof(buffer), chat->c_str(), guildname.c_str());
                    g_interface->ShowBlueNotify(buffer);

                    sysmsg->WriteMessage(0xFF, DEFAULT_SYSTEM_COLOR, buffer, 1, 1);
                    break;
                }

                case 0x0C08:
                {
                    g_interface->OnNotifyAction(17, type, DEFAULT_SYSTEM_COLOR, 0, 1);
                    break;
                }

                case 0x0C10:
                {
                    chat = g_textmanager->GetStringTextByCode(L"UIIT_TQ_F4_TELEPORT_OPEN");
                    g_interface->ShowBlueNotify(chat);

                    sysmsg->WriteMessage(0xFF, DEFAULT_SYSTEM_COLOR, chat->c_str(), 1, 1);
                    break;
                }

                case 0x0C11:
                {
                    chat = g_textmanager->GetStringTextByCode(L"UIIT_TQ_F4_TELEPORT_CLOSE");
                    g_interface->ShowBlueNotify(chat);
                    sysmsg->WriteMessage(0xFF, DEFAULT_SYSTEM_COLOR, chat->c_str(), 1, 1);
                    break;
                }
                ...
                case 0x0C12:
                {
                    chat = g_textmanager->GetStringTextByCode(L"UIIT_MSG_QUEST_ROC_APPEAR");
                    g_interface->ShowBlueNotify(chat);
                    sysmsg->WriteMessage(0xFF, DEFAULT_SYSTEM_COLOR, chat->c_str(), 1, 1);
                    break;
                }
                ...
                case 0x0C13:
                {
                    chat = g_textmanager->GetStringTextByCode(L"UIIT_MSG_QUEST_ROC_DISAPPEAR");
                    g_interface->ShowBlueNotify(chat);
                    sysmsg->WriteMessage(0xFF, DEFAULT_SYSTEM_COLOR, chat->c_str(), 1, 1);
                    break;
                }
                ...
                case 0x0C14:
                {
                    char mins = msg->Read<char>();
                    switch (mins)
                    {
                        case 0:
                        {
                            chat = g_textmanager->GetStringTextByCode(L"UIIT_MSG_QUEST_ROC_BERSERKER_START");
                            g_interface->ShowBlueNotify(chat);
                            sysmsg->WriteMessage(0xFF, DEFAULT_SYSTEM_COLOR, chat->c_str(), 1, 1);
                            break;
                        }
                        ...
                        case -1:
                        {
                            chat = g_textmanager->GetStringTextByCode(L"UIIT_MSG_QUEST_ROC_BERSERKER_AGAIN");
                            g_interface->ShowBlueNotify(chat);
                            sysmsg->WriteMessage(0xFF, DEFAULT_SYSTEM_COLOR, chat->c_str(), 1, 1);
                            break;
                        }
                        ...
                        default:
                        {
                            chat = g_textmanager->GetStringTextByCode(L"UIIT_MSG_QUEST_ROC_BERSERKER");
                            wchar_t buffer[0x100];
                ...
                {
                    wchar_t buffer[0x100];
                    chat = g_textmanager->GetStringTextByCode(L"UIIT_MSG_QUEST_ROC_BYEBYE");
                    swprintf_s(buffer, sizeof(buffer), chat->c_str(), msg->Read<char>());
                    g_interface->ShowBlueNotify(buffer);
                    sysmsg->WriteMessage(0xFF, DEFAULT_SYSTEM_COLOR, buffer, 1, 1);
                    break;
                }
                ...
                case 0x0C16:
                {
                    chat = g_textmanager->GetStringTextByCode(&msg->ReadStringW());
                    g_interface->HandleSystemMessages(0, chat->c_str());
                    g_interface->ShowBlueNotify(chat);
                ...
                case 0x0C17:
                {
                    ...
                    unsigned char flag = msg->Read<unsigned char>();
                    ...
                    switch (flag)
                    {
                        case 0:
                        {
                            fullchat.assign(*g_textmanager->GetStringTextByCode(L"SN_ZONE_21837_2"));
                            fullchat.append(L"]/[");
                            fullchat.append(*g_textmanager->GetStringTextByCode(L"SN_ZONE_21836_2"));
                            break;
                        }

                        case 1:
                        {
                            fullchat.assign(*g_textmanager->GetStringTextByCode(L"SN_ZONE_21836_4"));
                            break;
                        }

                        case 2:
                        {
                            fullchat.assign(*g_textmanager->GetStringTextByCode(L"SN_ZONE_21835_3"));
                            fullchat.append(L"]/[");
                            fullchat.append(*g_textmanager->GetStringTextByCode(L"SN_ZONE_21835_5"));
                            break;
                        }
                    }

                    swprintf_s(buffer, sizeof(buffer), g_textmanager->GetStringTextByCode(L"UIIT_MSG_EGYPT_SHRINE_ENTRANCE_ALARM")->c_str(), fullchat.c_str(), msg->Read<unsigned char>());
                ...
                case 0x0C18:
                {
                    ...
                    unsigned char flag = msg->Read<unsigned char>();
                    unsigned char uniontype = msg->Read<unsigned char>();
                    ...
                    switch (uniontype)
                    {
                        case 0:
                        {
                            dunion.assign(*g_textmanager->GetStringTextByCode(L"UIIT_STT_EGYPT_SHRINE_ENTRANCE_UNION_EMPTY"));
                            break;
                        }

                        case 1:
                        {
                            dunion.assign(*g_textmanager->GetStringTextByCode(L"UIIT_PAG_COMPANY_MERCHANT"));
                            fullchat.append(L" / ");
                            break;
                        }
                        ...
                        case 2:
                        {
                            dunion.assign(*g_textmanager->GetStringTextByCode(L"UIIT_PAG_COMPANY_THIEF"));
                            break;
                        }

                        case 3:
                        {
                            dunion.assign(*g_textmanager->GetStringTextByCode(L"UIIT_STT_EGYPT_SHRINE_ENTRANCE_UNION_ALL"));
                            break;
                        }
                    }

                    if (flag == 1 && !uniontype)
                        swprintf_s(buffer, sizeof(buffer), g_textmanager->GetStringTextByCode(L"UIIT_MSG_EGYPT_SHRINE_ENTRANCE_UNION_ALARM_04")->c_str(), g_textmanager->GetStringTextByCode(L"UIIT_STT_EGYPT_SHRINE_ENTRANCE_UNION_ALL")->c_str(), fullchat.c_str());
                    else
                        swprintf_s(buffer, sizeof(buffer), g_textmanager->GetStringTextByCode(L"UIIT_MSG_EGYPT_SHRINE_ENTRANCE_UNION_ALARM_01")->c_str(), dunion.c_str(), fullchat.c_str());
                    ...
                    if (flag < 2)
                    {
                        ...
                        if (CGlobalDataManager* g_datamanager = CGlobalDataManager::Get())
                        {
                            ...
                            if (g_interface->sub_799970())
                                g_interface->sub_799970();
                        }
                    }
                    break;
                }
                ...
                case 0x0C19:
                {
                    ...
                    if (flag < 2)
                    {
                        unsigned char templething = msg->Read<unsigned char>();
                ...
                case 0x0C1A:
                {
                    chat = g_textmanager->GetStringTextByCode(L"UIIT_MSG_EGYPT_DUNGEON_DESTROY_ALARM");
                    wchar_t buffer[0x100];
                    swprintf_s(buffer, sizeof(buffer), chat->c_str(), msg->Read<char>());
                    g_interface->ShowBlueNotify(buffer);
                    sysmsg->WriteMessage(0xFF, DEFAULT_SYSTEM_COLOR, buffer, 1, 1);
                ...
                case 0x0C1B:
                {
                    unsigned short WorldID = msg->Read<unsigned short>();
                    ...
                    if (CGlobalDataManager* g_datamanager = CGlobalDataManager::Get())
                    {
                        ...
                        if (WorldCode.compare(L"INS_PHARAOH_NOVICE") == 0)
                            swprintf_s(buffer, sizeof(buffer), g_textmanager->GetStringTextByCode(L"UIIT_MSG_EGYPT_TOMB_ENTRANCE_CONDITION_ERROR_04")->c_str(), g_textmanager->GetStringTextByCode(L"SN_ZONE_21836_10")->c_str());
                        else if (WorldCode.compare(L"INS_PHARAOH_EXPERT") == 0)
                            swprintf_s(buffer, sizeof(buffer), g_textmanager->GetStringTextByCode(L"UIIT_MSG_EGYPT_TOMB_ENTRANCE_CONDITION_ERROR_04")->c_str(), g_textmanager->GetStringTextByCode(L"SN_ZONE_21836_11")->c_str());
                        else if (WorldCode.compare(L"INS_PHARAOH_MASTER") == 0)
                            swprintf_s(buffer, sizeof(buffer), g_textmanager->GetStringTextByCode(L"UIIT_MSG_EGYPT_TOMB_ENTRANCE_CONDITION_ERROR_04")->c_str(), g_textmanager->GetStringTextByCode(L"SN_ZONE_21836_12")->c_str());
                        else
                ...
                case 0x0C1C:
                {
                    ...
                    switch (flag)
                    {
                        case 1:
                        {
                            chat = g_textmanager->GetStringTextByCode(&msgcode);
                            g_interface->ShowBlueNotify(chat);
                            break;
                        }
