...
const DWORD dwHookSlash = 0x00a60f90;
...
void CICUser::Func_16() {
    ...
    reinterpret_cast<void(__thiscall*)(CICUser*)>(0x009db940)(this);
}

void CICUser::FUN_009dae90(BYTE &a1) {
    /// 009dae90
    reinterpret_cast<void (__thiscall *)(CICUser *, BYTE *)>(0x009dae90)(this, &a1);
}
...
void CICUser::Func_15(int param_1, float *param_2) {
   ...
   // printf("Func_15 %d %p\n", param_1, param_2);
    ...
    if (CGame::STA_FUN_004f9d00().field_c != '\0') {
        ...
        {
            //printf("GetName %S \n",this->GetName().c_str());
            const IDirect3DBaseTexture9* puVarxxx ;
            std::map<std::wstring, UINT32>::iterator foundedInfo = m_CharacterIcons.find(this->GetName().c_str());
            if (foundedInfo != m_CharacterIcons.end())
            {
                std::map<int, const IDirect3DBaseTexture9*>::iterator foundedTheIcon = m_IconsDataStore.find(foundedInfo->second);
                if (foundedTheIcon != m_IconsDataStore.end())
                {
                    puVarxxx = foundedTheIcon->second;
                }
                else
                {
                    puVarxxx = NULL; //p_MiniIcon_TexturePtrs[0x4] ;
                }
            }
            else
            {
            ...
            if(puVarxxx != NULL)
            {
                g_RStateMgr.SetTextureForStage(0, puVarxxx); // icon\\etc\\icon_rudiment.ddj
                int local_158 = 0;
                if (g_pDynamicVertexBuffer->IVBDynamic_Func_6(dataOut, 0x60, &local_158) != 0) {
                    IDirect3DVertexBuffer9 *iVar5 = g_pDynamicVertexBuffer->IVBDynamic_Func_5();
                    g_RStateMgr.m_pDevice->SetStreamSource(0, iVar5, 0, 0x18);
            ...
            const IDirect3DBaseTexture9* puVarxx;

            std::map<std::wstring, UINT32>::iterator foundedVipInfo = m_CharacterVipIcons.find(this->GetName().c_str());
            if (foundedVipInfo != m_CharacterVipIcons.end())
            {
                if(foundedVipInfo->second > 0)
                {
                    switch (foundedVipInfo->second )
                    {
                        case 1:
                        {
                            puVarxx = Fun_CacheTexture_Create_3D("icon\\vip\\vip_01.ddj");
                        }
                            break;
                        case 2:
                        {
                            puVarxx = Fun_CacheTexture_Create_3D("icon\\vip\\vip_02.ddj");
                        }
                            break;
                        case 3:
                        {
                            puVarxx = Fun_CacheTexture_Create_3D("icon\\vip\\vip_03.ddj");
                        }
                            break;
                        case 4:
                        {
                            puVarxx = Fun_CacheTexture_Create_3D("icon\\vip\\vip_04.ddj");
                        }
                            break;
                        case 5:
                        {
                            puVarxx = Fun_CacheTexture_Create_3D("icon\\vip\\vip_05.ddj");
                        }
                            break;
                        case 6:
                        {
                            puVarxx = Fun_CacheTexture_Create_3D("icon\\vip\\vip_06.ddj");
                        }
                            break;
                    }
                }
                ...
                {
                    if(this->GetObjID() >= 14875 && this->GetObjID() <= 14900)
                        puVarxx = Fun_CacheTexture_Create_3D("interface\\ifcommon\\com_kindred_europe.ddj");
                    if(this->GetObjID() >= 1907 && this->GetObjID() <= 1932)
                        puVarxx = Fun_CacheTexture_Create_3D("interface\\ifcommon\\com_kindred_china.ddj");
            ...
            if(puVarxx != NULL)
            {
                ...
                dataOut[6].x = dataOut[0].x;
                int local_159 = 0;
                if (g_pDynamicVertexBuffer->IVBDynamic_Func_6(dataOut, 0x60, &local_159) != 0) {
                    IDirect3DVertexBuffer9 *iVar5 = g_pDynamicVertexBuffer->IVBDynamic_Func_5();
                    g_RStateMgr.m_pDevice->SetStreamSource(0, iVar5, 0, 0x18);
