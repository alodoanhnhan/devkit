...
void CIFTargetWindowJobPlayer::FUN_0069ac90(int objectId) {
   ...
   const wchar_t *title = NULL;
    int trijobType = pUser->GetJobTypes();
    ...
    p_JOB_GRADENAME->SetText(TSM_GETTEXTPTR1(buffer));

    m_IRM.GetResObj<CIFStatic>(GDR_TWJP_JOB_GRADE, 1)->SetTextFormatted(L"%d %s", pUser->GetJobLevel(), TSM_GETTEXTPTR1(L"UIIT_STT_GRADE"));
}
